...
if (isset($_GET['delete'])) {
  ...
  $stmt = $conn->prepare("SELECT foto FROM notes WHERE id = ? AND user_id = ? AND category_id = 2");
  ...
  if ($result->num_rows > 0) {
    ...
    $stmt = $conn->prepare("DELETE FROM notes WHERE id = ? AND user_id = ? AND category_id = 2");
    $stmt->bind_param("ii", $delete_id, $user_id);
    $stmt->execute();
...
// Fetch Notes with category_id = 2 (Work)
$stmt = $conn->prepare("SELECT * FROM notes WHERE user_id = ? AND category_id = 2 ORDER BY created_at DESC");
...
<head>
  ...
  <title>Notes Work - Sprinklist</title>
...
<body>
  ...
  <div class="sidebar">
    ...
    <div class="submenu active-menu" id="notes-submenu">
      ...
      <a href="notes/work.php" style="background-color: #f8d7d7; color: #7a4e2f;"><i class="fa-solid fa-file-lines"></i> Work</a>
  ...
  <script>
    ...
    function confirmDelete(id) {
      if (confirm('Apakah Anda yakin ingin menghapus note ini?')) {
        window.location.href = 'notes/work.php?delete=' + id;
      }
    }
  </script>
